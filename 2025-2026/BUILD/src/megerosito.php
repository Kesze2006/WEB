...
if (!$token) {
    ...
    WHERE token = ?
      AND tipus = $tipus _megerosites
      AND felhasznalva = 0
      AND lejarat > NOW()
");
    $check->execute([$token]);
    $user = $check->fetch(PDO::FETCH_ASSOC);
    $update_felhasznalo = $adatBazis->prepare("UPDATE felhasznalo SET $tipus _megerositve = 1  WHERE id = ?
    ");
    ...
    if ($check->rowCount() === 1) {
        ...
        echo json_encode(["success" => "Siker!", "$tipus _megerositve" => true], JSON_UNESCAPED_UNICODE);
    } else {
